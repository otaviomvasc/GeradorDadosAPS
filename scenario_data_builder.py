...
class ScenarioDataBuilder():
    ...
    def match_points_to_sectors(self, df_divi):
        ...
        self.df_setor_censitario['geometry'] = self.df_setor_censitario['coordinates'].apply(parse_polygon)
        ...
        df_divi,
        geometry=gpd.points_from_xy(df_divi['NU_LONGITUDE'], df_divi['NU_LATITUDE']),
        crs='EPSG:4326'
        )
    ...
    def read_and_format_path_setores_com_UBS(self):
        ...
        self.df_setor_censitario["CO_UNIDADE_UBS"] = self.df_setor_censitario["CO_UNIDADE_UBS"].fillna(0)
        self.df_setor_censitario["CO_UNIDADE"] = self.df_setor_censitario["CO_UNIDADE_UBS"] #TODO: Avaliar isso com calma depois o motivo de ter trocado para CO_UNIDADE_UBS e nao ter quebrado nada!
    ...
    def complete_CO_unidade(self, df):
        dict_aux = {cnes: max(df.loc[df.CO_CNES == cnes].CO_UNIDADE.to_list()) for cnes in df.CO_CNES}
    ...
    def agg_IVS_por_cluster(self):
        ...
        nan_mask = df_agg_IVS['Índice'].isna()
        nan_indices = df_agg_IVS[nan_mask].index
        
        df_agg_IVS.loc[nan_indices, "Índice"] = "."
        return df_agg_IVS
    ...
    def  set_cluster_to_be_candidate_locations(self):
        ...
        self.df_setor_censitario["IS_CL"] = self.df_setor_censitario.SETOR.apply(lambda x: True if x in candidate_locations else False)
        self.cluster_CL = self.df_setor_censitario[self.df_setor_censitario["IS_CL"] == True].cluster.unique()
    ...
    def set_full_PHC_locations(self):
        ...
        self.df_PHC_by_cluster =  self.df_PHC_by_cluster[~self.df_PHC_by_cluster.CO_UNIDADE.isna()].reset_index()
    ...
    def cluster_CL_coordinates(self):
        self.df_clusters_candidates_PHC_locations = self.df_agg_cluster[self.df_agg_cluster.cluster.isin(self.cluster_CL)][["cluster", "LAT", "LONG"]].copy()
        #self.df_clusters_candidates_PHC_locations["CO_UNIDADE"] = self.df_clusters_candidates_PHC_locations.cluster.apply(lambda x: f"CL_cluster_{int(x)}")
        self.df_clusters_candidates_PHC_locations["CO_UNIDADE"] = self.df_clusters_candidates_PHC_locations.cluster
    ...
    def read_and_format_SIZE_PHC(self):
        ...
        self.df_setor_censitario.loc[
            (self.df_setor_censitario.CO_UNIDADE > 0) & (self.df_setor_censitario.PORTE_UBS.isna()),
            'PORTE_UBS'
        ] = 1
            
    def read_and_format_CL(self):
        try:
            df_CL = pd.read_excel(self.path_arquivos_data.path_locais_candidatos, sheet_name="Candidatos_Finais")
        except: 
    ...
    def add_clusters_to_PHC_candidates(self, clusters_without_PHC):
        ...
        for cl in clusters_without_PHC:
            data = {"CO_UNIDADE": cl, "cluster": cl, 
            "PORTE_UBS": 0, 
            "LAT": self.df_agg_cluster[self.df_agg_cluster.cluster == cl].LAT.values[0],
            "LONG": self.df_agg_cluster[self.df_agg_cluster.cluster == cl].LONG.values[0]}
            final_new_CL_data.append(data)

        new_CL_df = pd.DataFrame(final_new_CL_data)
        self.df_full_PHC_locations = pd.concat([self.df_full_PHC_locations,
                                                new_CL_df], ignore_index=True).reset_index(drop=True)
